...
export default function SavedOpportunitiesPage() {
    ...
    const fetchSaved = async () => {
        ...
        try {
            ...
            if (data.success) {
                setItems(data.items.filter((item) => item.itemType === 'opportunity'));
            }
        } catch (e) {
            console.error("Failed to load saved opportunities:", e);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-black text-white p-6 md:p-16">
            <div className="pb-8">
                <div className="flex gap-3 mb-4">
                    <Link href="/dashboard/saved/startups" className="px-4 py-2 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 text-sm font-semibold hover:text-white transition-colors">
                        Startups
                    </Link>
                    <Link href="/dashboard/saved/opportunities" className="px-4 py-2 rounded-full bg-purple-600 text-white text-sm font-semibold">
                        Opportunities
                    </Link>
                </div>
                <h1 className="text-4xl font-extrabold tracking-tight mb-2">Saved Opportunities</h1>
                <p className="text-zinc-400">Opportunities you've bookmarked for later.</p>
            </div>

            {items.length === 0 ? (
                <div className="text-center py-16 bg-zinc-950 border border-zinc-900 rounded-xl text-zinc-500">
                    Nothing saved yet — tap the heart icon on any opportunity to save it here.
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {items.map((item) => (
                        ...
                        >
                            ...
                            <Link href={`/opportunities/${item._id}`}>
