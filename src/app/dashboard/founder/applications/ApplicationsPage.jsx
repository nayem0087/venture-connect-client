...
import { getAllApplications, updateApplicationStatus, getAllOpportunities } from '@/lib/actions/jobs';
import Loading from "@/components/Loading";

export default function ApplicationsPage({ founder }) {
    ...
    useEffect(() => {
        async function loadApplications() {
            try {
                ...
                // ===== DIAGNOSTIC START =====
                // console.log("%c=== ApplicationsPage diagnostic ===", "color: orange; font-weight: bold");
                // console.log("1. founder prop:", founder);
                // console.log("2. founderEmail derived:", founderEmail);
                // ===== DIAGNOSTIC END =====

                if (!founderEmail) {
                    console.warn("ApplicationsPage: 'founder.email' is missing", { founder });
                    setApplications([]);
                    return;
                }
                ...
                // ===== DIAGNOSTIC =====
                // console.log("3. getAllOpportunities() raw result:", oppRes);
                // ===== DIAGNOSTIC =====
                ...
                const myJobIds = new Set(
                    oppRes.data
                        .filter((job) => job.founderEmail === founderEmail)
                        .map((job) => String(job._id))
                );

                // ===== DIAGNOSTIC =====
                // console.log("4. All opportunities (id + founderEmail):",
                //     oppRes.data.map(j => ({ id: String(j._id), founderEmail: j.founderEmail, title: j.title }))
                // );
                // console.log("5. myJobIds (this founder's own job ids):", Array.from(myJobIds));
                // ===== DIAGNOSTIC =====

                const res = await getAllApplications();

                // ===== DIAGNOSTIC =====
                // console.log("6. getAllApplications() raw result:", res);
                // ===== DIAGNOSTIC =====
                ...
                if (res?.success) {
                    ...
                    // ===== DIAGNOSTIC =====
                    // console.log("7. All applications (full raw objects):", allApplications);
                    // console.log("8. All applications (id + opportunityId + jobId, whichever exist):",
                    //     allApplications.map(a => ({
                    //         id: String(a._id),
                    //         opportunityId: a.opportunityId !== undefined ? String(a.opportunityId) : '(missing)',
                    //         jobId: a.jobId !== undefined ? String(a.jobId) : '(missing)',
                    //     }))
                    // );
                    // ===== DIAGNOSTIC =====

                    const myApplications = allApplications.filter((app) =>
                        myJobIds.has(String(app.opportunityId))
                    );

                    // ===== DIAGNOSTIC =====
                    // console.log(`9. RESULT: filtered ${myApplications.length} of ${allApplications.length} total applications.`);
                    // console.log("%c=== end diagnostic ===", "color: orange; font-weight: bold");
                    // ===== DIAGNOSTIC =====
    ...
    const handleStatusUpdate = async (id, newStatus) => {
        try {
            // console.log("%c--- handleStatusUpdate called ---", "color: cyan; font-weight: bold");
            // console.log("id:", id, "newStatus:", newStatus);

            const res = await updateApplicationStatus(id, newStatus);

            // console.log("updateApplicationStatus() raw response:", res);
            ...
            if (res?.success) {
                ...
                // console.warn("Update failed — server did not return success:", res);
                toast.error("Failed to update status");
            }
        } catch (err) {
            console.error("handleStatusUpdate threw an error:", err);
    ...
    const getStatusStyle = (status) => {
        switch (status?.toLowerCase()) {
            case 'accepted':  return 'bg-emerald-950/40 text-emerald-400 border-emerald-900/50';
            case 'rejected':  return 'bg-rose-950/40 text-rose-400 border-rose-900/50';
            default:          return 'bg-amber-950/40 text-amber-400 border-amber-900/50';
        }
    };
                            ...
                            <div className="space-y-4 w-full md:max-w-[70%]">
                                <div className="flex items-center gap-2 flex-wrap">
                                    <h3 className="text-lg font-semibold text-white tracking-wide">
                                        {item.opportunityTitle || item.title || item.jobTitle}
                                    </h3>
                                ...
                                <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-zinc-500">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>{item.applicantEmail || item.email}</span>
                                    </div>
                                    <div className="flex items-center gap-1.5">
                                        <Calendar size={13} className="text-zinc-600" />
                                        <span>Applied: {item.appliedAt ? new Date(item.appliedAt).toLocaleDateString() : item.createdAt ? new Date(item.createdAt).toLocaleDateString() : 'N/A'}</span>
                                    </div>
                                </div>
                                ...
                                {(item.coverLetter || item.motivation) && (
                                    <div className="space-y-1 pt-1">
                                        <span className="text-[10px] uppercase font-bold tracking-wider text-zinc-600 block">
                                            {item.coverLetter ? "Cover Letter" : "Motivation"}
                                        </span>
