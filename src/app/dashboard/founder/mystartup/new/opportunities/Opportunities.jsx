...
export default function OpportunitiesPage({ founder }) {
    const [jobs, setJobs] = useState([]);
    const [isLoading, setIsLoading] = useState(true);
    const [editingId, setEditingId] = useState(null);

    // The opportunity documents store "founderEmail" (not founderId), so
    // that's what we match against to find this founder's own postings.
    const founderEmail = founder?.email;

    const getOwnerEmail = (job) => job.founderEmail || null;
    ...
    useEffect(() => {
        async function loadJobs() {
            try {
                ...
                if (res.success) {
                    if (founderEmail) {
                        const myJobs = res.data.filter(
                            (job) => getOwnerEmail(job) === founderEmail
                        );
                        setJobs(myJobs);

                        if (myJobs.length === 0 && res.data.length > 0) {
                            console.warn(
                                "OpportunitiesPage: no opportunities matched founderEmail — check shapes below for a mismatch.",
                                {
                                    founderEmailFromProp: founderEmail,
                                    sampleJobRaw: res.data[0],
                                    sampleJobOwnerEmailExtracted: getOwnerEmail(res.data[0]),
                                }
                            );
                        }
                    } else {
                        // 'founder' prop missing (or has no email) — fail
                        // safe to an empty list rather than ever showing
                        // everyone's opportunities, and warn loudly so it's
                        // easy to spot and fix in the parent server page.
                        console.warn(
                            "OpportunitiesPage: 'founder.email' is missing — cannot filter by owner, showing none.",
                            { founder }
                        );
                        setJobs([]);
                    }
                ...
                // Always resolves, regardless of whether founderEmail is
                // present — avoids ever getting stuck on the loading screen.
                setIsLoading(false);
            }
        }
        loadJobs();
