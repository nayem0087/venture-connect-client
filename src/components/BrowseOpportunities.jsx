        ...
        <div className="min-h-screen bg-black text-white p-6 md:p-16">
            {/* Header Section */}
                ...
                <div className='text-center pb-10'>
                    <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4 bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-500">
                        Open Opportunities
                    </h1>
            ...
            >
                {loading ? (
                    [...Array(6)].map((_, i) => (
                        <div key={i} className="h-64 bg-zinc-900/30 rounded-3xl animate-pulse" />
                    ))
                ) : (
                    opportunities.map((opp) => (
                        ...
                        >
                            ...
                            <div className="relative z-10">
                                ...
                                <p className="text-zinc-500 text-sm mb-6">{opp.email}</p>

                                {/* Tags */}
                                <div className="flex flex-wrap gap-2 mb-8">
                                    {opp.skills?.split(',').map((skill, i) => (
                                        <span key={i} className="px-3 py-1 bg-zinc-900 rounded-lg text-[11px] font-medium text-zinc-400 border border-zinc-800 uppercase tracking-wider">
                                            {skill.trim()}
                                        </span>
                                    ))}
                                </div>
